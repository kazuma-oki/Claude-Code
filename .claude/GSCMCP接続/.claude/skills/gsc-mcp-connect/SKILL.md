...
プロパティ一覧が返ってきたら、表示された `site_url`（例: `https://n2p.co.jp/`）を使って動作確認する:
```
https://n2p.co.jp/ の過去7日間の検索クエリ上位10件を教えて
```

> **注意:** `sc-domain:ドメイン名` 形式ではなく、`list_properties` で返ってきた `site_url` の値をそのまま使うこと。形式が違うと 403 エラーになる。
